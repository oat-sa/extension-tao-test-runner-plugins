...
var itemRunner=testRunner.itemRunner,key=testRunner.getTestContext().itemIdentifier;if(itemRunner&&!currentItem.isAnswered(testRunner))return itemRunner.on("responsechange",function(){self.storage.setItem(key,itemRunner.getState())}),self.storage.getItem(key).then(function(state){state&&itemRunner.setState(state)})}).on("unloaditem",function(){if(!testRunner.getState("closedOrSuspended"))return self.storage.clear()})})}})}),define("taoTestRunnerPlugins/runner/plugins/navigation/limitBackButton",["jquery","lodash","taoTests/runner/plugin"],function($,_,pluginFactory){"use strict";var pluginName="limitBackButton",customListeners=[{selector:".widget-numpad",
eventName:"click"}],getItemKey=function(itemIdentifier){return itemIdentifier+"-limit-back-button"},removeAllListeners=function(testRunner){var areaBroker=testRunner.getAreaBroker();_.forEach(customListeners,function(plugin){areaBroker.getContainer().find(plugin.selector).off("."+pluginName)}),areaBroker.getContentArea().find(".qti-interaction").off("."+pluginName)};return pluginFactory({name:pluginName,init:function(){var testRunner=this.getTestRunner();return testRunner.getPluginStore(this.getName()).then(function(store){var disableState=!1,processingMoveAction=!1,toggleBackButton=function(){var previous=testRunner.getPlugin("previous")
;disableState?previous.disable():previous.enable()},createListener=function($element,eventName){var event=eventName?eventName+"."+pluginName:"change."+pluginName;$element.on(event,function(){testRunner.itemRunner.trigger("responsechange."+pluginName)})};testRunner.before("move."+pluginName,function(e,direction){if(processingMoveAction=!0,"previous"===direction&&disableState)return Promise.reject()}),testRunner.after("enablenav."+pluginName,function(){toggleBackButton()}),testRunner.after("disablenav."+pluginName,function(){disableState&&testRunner.trigger("enablenav")}),testRunner.after("renderitem",function(itemIdentifier){var $itemInteractions,self=this,
itemRunner=testRunner.itemRunner;processingMoveAction=!1,itemRunner.on("responsechange."+pluginName,function(){processingMoveAction||(disableState=!0,store.setItem(getItemKey(itemIdentifier)).then(toggleBackButton))}),_.forEach(customListeners,function(plugin){var $element=self.getAreaBroker().getContainer().find(plugin.selector);createListener($element,plugin.eventName)}),$itemInteractions=self.getAreaBroker().getContentArea().find(".qti-interaction"),createListener($itemInteractions)}).on("loaditem",function(itemIdentifier){store.getItem(getItemKey(itemIdentifier)).then(function(val){disableState=val,toggleBackButton()})}).on("beforeunloaditem",function(){
removeAllListeners(testRunner)})})},destroy:function(){removeAllListeners(this.getTestRunner())}})}),define("taoTestRunnerPlugins/runner/plugins/probes/latency",["lodash"],function(_){"use strict";var latencyProbes,timers={},captureProcessors={captureTest:function(testRunner){return{testId:testRunner.getTestData().identifier}},captureAll:function(testRunner){var data=testRunner.getTestData(),context=testRunner.getTestContext();return{testId:data.identifier,testPartId:context.testPartId,sectionId:context.sectionId,itemId:context.itemIdentifier,sectionTimer:latencyProbes.getTimerValue("assessmentSection"),attempt:context.attempt}},captureShortcut:function(
testRunner,eventName,shortcut){return _.assign(latencyProbes.applyCaptureProcessor("captureAll",testRunner),{shortcut:shortcut})}};return latencyProbes={init:function(testRunner,probesConfig){var probeOverseer=testRunner.getProbeOverseer();function sendVariables(){return probeOverseer.flush().then(function(data){var traceData={};return data&&data.length&&(_.forEach(data,function(entry){var id=entry.type+"-"+entry.id;entry.marker&&(id=entry.marker+"-"+id),traceData[id]=entry}),testRunner.getProxy().sendVariables(traceData,!0)),data})}return probesConfig.forEach(function(probe){probeOverseer.add({name:probe.name,events:probe.events,latency:probe.latency,
startEvents:probe.startEvents,stopEvents:probe.stopEvents,capture:captureProcessors[probe.capture]})}),testRunner.on("plugin-addtimer.timer",function(timerPlugin,type,timer){timers[type]=timer}),testRunner.on("plugin-removetimer.timer",function(timerPlugin,type){timers[type]=!1}),testRunner.before("unloaditem",function(){return sendVariables()}),testRunner.before("exit",function(){return sendVariables()}),this},getTimerValue:function(name){if("object"==typeof timers[name]){if(_.isNumber(timers[name].remainingTime))return timers[name].remainingTime;if(_.isFunction(timers[name].val))return parseInt(timers[name].val(),10)}return null},
